...
if (not CONFIG_PATH.joinpath("config.yaml").exists()
    ...
    logger.error("请修改 config.exp.yaml 并重命名为 config.yaml ！")
    exit()
elif (not CONFIG_PATH.joinpath("config.yaml").exists()
    ...
    with CONFIG_PATH.joinpath("group_key.yaml").open("r",
                                                     encoding="utf-8") as f:
        key_data = f.read()
    group_key = yaml.load(key_data, Loader=yaml.FullLoader)

if CONFIG_PATH.joinpath("groupdata.json").exists():
    with CONFIG_PATH.joinpath("groupdata.json").open("r",
                                                     encoding="utf-8") as f:
        group_data = json.load(f)
else:
    with CONFIG_PATH.joinpath("groupdata.json").open("w",
                                                     encoding="utf-8") as f:
        group_data = {}
        json.dump(group_data, f, indent=2)

if CONFIG_PATH.joinpath("grouplist.json").exists():
    with CONFIG_PATH.joinpath("grouplist.json").open("r",
                                                     encoding="utf-8") as f:
        group_list = json.load(f)
else:
    with CONFIG_PATH.joinpath("grouplist.json").open("w",
                                                     encoding="utf-8") as f:
...
if CONFIG_PATH.joinpath("userlist.json").exists():
    with CONFIG_PATH.joinpath("userlist.json").open("r",
                                                    encoding="utf-8") as f:
        user_list = json.load(f)

else:
    with CONFIG_PATH.joinpath("userlist.json").open("w",
                                                    encoding="utf-8") as f:
        user_list = {"black": []}
...
if (yaml_data["Basic"]["Permission"]["Master"]
...
def change_config(data):
    try:
        with CONFIG_PATH.joinpath("config.yaml").open("w",
                                                      encoding="utf-8") as f:
