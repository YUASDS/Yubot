...
async def group_learn(group: Group, member: Member):
    ...
    async def confirm(waiter_group: Group, waiter_member: Member,
    ...
    @Waiter.create_using_function(listening_events=[GroupMessage],
                                  using_decorators=[Permission.require()])
    async def waiter(waiter_group: Group, waiter_member: Member,
                     waiter_message: MessageChain):
    ...
    while True:
        ...
        for process in Process:
            try:
                answer_qq = await asyncio.wait_for(inc.wait(waiter),
                                                   timeout=15)
                if answer_qq:
...
async def friend_learn(app: Ariadne, friend: Friend):
    ...
    try:
        ...
        del RUNNING[friend.id]
        return await app.sendFriendMessage(
            friend, MessageChain.create([Plain("等待超时")]))
