...
@channel.use(
    ListenerSchema(
        listening_events=[GroupMessage],
        inline_dispatchers=[
            Twilight({"head": RegexMatch("WaifuLab|Lab头像生成|Lab获取头像|Lab头像"),
...
async def WaifuLab(group: Group,member:Member,water:RegexMatch):
    ...
    if BROWER>2:
        ...
        group, MessageChain.create("当前进程过多，等一会再来哦~" ))
    else :
        BROWER+=1
    if water.matched:
    ...
    if not stat:
        BROWER-=1
        return    await safeSendGroupMessage(
        group, MessageChain.create("连接超时，等一会再来哦~" ))
    pic=await page.first_shot()
    pic=pic.getvalue()
    await safeSendGroupMessage(
        group, MessageChain.create("连接成功，请选择下一步哦~" ,Image(data_bytes=pic)))
    func_dict={
        "选择":page.other_shot,
        "继续":page.continues,
        "返回":page.back,
    ...
    @Waiter.create_using_function(listening_events=[GroupMessage],
        using_dispatchers=[
            Twilight({"head": RegexMatch("选择|继续|返回|退出"),
    ...
    async def get_waifu(waiter_member:Member,head:RegexMatch,choose:RegexMatch):
            if member.id==waiter_member.id:
                ...
                if choose.matched:
                    ...
                    if choose>16:
                        await safeSendGroupMessage(
        group, MessageChain.create("超出选择范围了哦~"))
                        return
                    if choose==0:
                        await safeSendGroupMessage(
        group, MessageChain.create("没有0这个选项哦~"))
                        return
                    img=func_dict[head]
                    ...
                    if not img:
                        ...
                        await safeSendGroupMessage(
        group, MessageChain.create("获取成功~",Image(data_bytes=img[0].getvalue()),Image(data_bytes=img[1].getvalue())))
                        return True
                    else:
                        await safeSendGroupMessage(
        group, MessageChain.create("头像获取成功~",Image(data_bytes=img.getvalue())))
    ...
    while sign:
        try:
            ...
            BROWER-=1
            return    await safeSendGroupMessage(
        group, MessageChain.create("等待超时~" ))
    BROWER-=1

    await safeSendGroupMessage(
        group, MessageChain.create("你需要的头像已经生成好了哦~"))
    pass