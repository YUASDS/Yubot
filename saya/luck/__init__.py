...
async def luck_draw(group: Group, member: Member):
    ...
    if await luckDB.get_user_time(qq=qq):
        ...
        await safeSendGroupMessage(group, MessageChain.create(f"你抽到了id为{luck_id}的盒装乌帕，其中有{gold}个乌帕，计算增益{change},共获得{total}个乌帕"))
...
async def luck(group: Group, member: Member):
    ...
    if (
        ...
        return
         ...
         return
    await safeSendGroupMessage(group, MessageChain.create("——————乌帕抽奖————\n通过放下一个乌帕然后捡起一个乌帕的简单抽奖~\n指令：\n放乌帕+数字（如：放乌帕15）（放乌帕会收取2手续费哦~）\n抽乌帕"))
