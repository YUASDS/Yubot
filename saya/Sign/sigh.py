...
async def get_signin_img(
    ...
    qlogo = Image.open(BytesIO(await get_qlogo(id=qq)))
    ...
    mahojin = Image.open(mahojin_path)
    ...
    qq_text = f"QQ：{qq}"
    uid = f"uid：{uuid}"
    impression = f"好感度：Lv{level}  {exp[0]} / {exp[1]}"
    ...
    #     draw.text((avatar_xy + 30, y), f"连续签到 {consecutive_days} 天",
    # font=font_3, fill=text_color)
    ...
    # hotokoto = '''由于一言目前属于公益性
    # 运营，为了保证资源的公平利用和不过度消耗公益资金，
    # 我们会定期的屏蔽某些大流量的站点。若您的站点的流量较大，
    # 您需要提前联系我们获得授权后再开始使用。对于超过阈值的站点，我们有可'''
